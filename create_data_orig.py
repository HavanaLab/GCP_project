...
def create_dataset(nmin, nmax, path, samples):
    ...
    while z in range(samples):
        ...
        if N not in instances: instances[N] = {}
        if Cn not in instances[N]: instances[N][Cn] = []
            ...
            if init_sol is not None and is_cn(Ma, Cn):
                ...
                for w in deg_rank:
                    ...
                    if diff_edge is not None:
                        ...
                        # Write graph to file
                        # write_graph(Ma, Ma, diff_edge, "{}/m{}.graph".format(path, z), False, cn=Cn,z=z)
                        instances[N][Cn].append([Ma, Cn, diff_edge])
                        print("created instance:" ,z)
                        z += 1
                        if (z - 1) % (samples // 10) == 0:
...
def write_graph(Ma, Mw, diff_edge, filepath, int_weights=False, cn=0, z=-1):
    dim = {"n": Ma.shape[0], "m": len(np.nonzero(Ma)[0]),  "cn": cn, "diff_edge": list(diff_edge), "mat": Ma.tolist()}
    data = pickle.dumps([dim])
    FS().upload_data(
        data, os.path.join("adversarial-training3", f'{Ma.shape[0]}/{cn}/N{Ma.shape[0]}_C{cn}_{z}.pkl')
    )
