...
def attributes_check(results_data, M_vv, M_vc, split,labels, cn, n, k, gcp, adj):
    ...
    local_assignments = []
    new_centroids = []
    for i in range(len(gcp.histories)):
        # assignment, _ = sklearn_k_means(adj, gcp.histories[i], k)
        assignment, centroid = sklearn_k_means(gcp.histories[i], k)
        ...
        votes.append((gcp.vote(gcp.histories[i], split)[0] > 0.5).item())
        # if disagree == 0:
        #     break
    ...
    for i in range(len(gcp.histories)):
        # assignment, _ = sklearn_k_means(adj, gcp.histories[i], k)
        assignment, new_centroid = sklearn_k_means(gcp.histories[i], k, centroids = centroid)
        is_k_col, disagree, missmatch_indexes = is_k_color(adj, assignment)
        # close_ass_iter = find_closest_kcoloring(assignment, adj, k)
        # miss_match_count = sum(x != y for x, y in zip(assignment, close_ass_iter))
        # closesdt_history.append(miss_match_count)
    ...
    results_data["closesed_index"].append(closesed_index)
    results_data["min_disagree"] = min_disagree[closesed_index]
    ...
    dist = sum(markers)
    results_data["closest_ass_dist"] = dist
    ...
    print("n",n, "k", k, "diss", min_disagree[closesed_index], "ind", closesed_index, "closest", dist, "pred_opt", pred_int, "label", int(labels), "acc_opt", int(pred_int == labels), "vote", votes[closesed_index], "mean_std_count", np.mean(votes), np.std(votes), np.sum(votes))
    ...
    df = create_dataframe_for_animation(
        embeddings_list,
        [str(d) if i not in [0,1,2] else str(-(i+10)) for i, d in enumerate(close_ass)],#[str(d) if i not in [1,2,3] else -i for i, d in enumerate(close_ass)], # [str(d != d2) for d, d2 in zip(assignments[closesed_index], close_ass)],
        closesed_index, assignments, adj, degrees, gcp, continues=True)
        ...
        try:
            ...
            for permute in permutations:
                ...
                if traingle_classification_metric > best_prediction:
                    ...
                    rectified_triangle = [triangle[i] for i in range(k)]

            print("triangle_metric", min_disagree[closesed_index], best_prediction)

        except Exception as e:
            print(e)
            print("triangle_metric", -2)
    else:
        print("triangle_metric", -1)
    ...
    if rectified_triangle is not None:
        for color in df['color'].unique():
            ...
            model = LinearRegression().fit([[0],[apex[0]]], [0, apex[1]])
            if len(X) < 2:
                r_sq =1
            else:
                r_sq = model.score(X, y)
    ...
    return
    # def average_neighbor_distance(adj, features):
    #     n = adj.shape[0]
    ...
    #             distance = np.linalg.norm(features[i] - features[neighbor])
    ...
    # prev_res = close_ass
    # pca = PCA(2)
    # pca.fit(gcp.histories[closesed_index].cpu().detach().numpy())
    # lower = pca.transform(gcp.histories[closesed_index].cpu().detach().numpy())
    # clause_lower = pca.transform(gcp.clauses_hist[closesed_index].reshape(1,-1).cpu().detach().numpy())
    #
    # nodes_avg_dist = average_neighbor_distance(adj.cpu().detach().numpy(), gcp.histories[closesed_index].cpu().detach().numpy()).argsort()[::-1]
    # nodes_avg_dist = np.argsort(np.linalg.norm(gcp.histories[closesed_index].cpu().detach().numpy(), axis=1)[::-1])
    # nodes_avg_dist = np.argsort(np.linalg.norm(gcp.histories[closesed_index].cpu().detach().numpy()-gcp.clauses_hist[closesed_index].cpu().detach().numpy(), axis=1)[::-1])
    # nodes_avg_dist = np.argsort(gcp.vote(gcp.histories[closesed_index], split)[-1][0].cpu().detach().numpy()[::-1])
    # nodes_avg_dist = np.argsort(np.linalg.norm(gcp.histories[closesed_index].cpu().detach().numpy()-np.linalg.norm(gcp.histories[closesed_index].cpu().detach().numpy().mean()), axis=1)[::-1])
    # nodes_avg_dist = np.argsort(np.linalg.norm(gcp.histories[closesed_index].cpu().detach().numpy()-gcp.histories[closesed_index].cpu().detach().numpy().mean(),axis=1)[::-1])
    #
    # nodes_avg_dist = average_neighbor_distance(adj.cpu().detach().numpy(), lower).argsort()[::-1]
    # nodes_avg_dist = np.argsort(np.linalg.norm(lower, axis=1)[::-1])
    # nodes_avg_dist = np.argsort(np.linalg.norm(lower-clause_lower, axis=1)[::-1])
    # nodes_avg_dist = np.argsort(gcp.vote(gcp.histories[closesed_index], split)[-1][0].cpu().detach().numpy()[::-1])
    # nodes_avg_dist = np.argsort(np.linalg.norm(lower-lower.mean(), axis=1)[::-1])

    # strong_confidence = []
    # base_ass = [-1] * n
    # for node in nodes_avg_dist:
    #     base_ass[node] = assignments[closesed_index][node]
    #     res = find_closest_kcoloring(assignments[closesed_index], adj, k, hint=base_ass)
    #     if res is None: break
    #     prev_res = res
    #     strong_confidence.append(node)
    # adj_copy = adj.clone()
    # neighbors = set()
    # cover = []
    # for sc in strong_confidence:
    #     adj_copy[:, sc] = adj_copy[sc, :] = 0
    #     if len(neighbors) < n:
    #         for e in torch.where(adj[sc]>0)[0]:
    #             neighbors.add(e.item())
    #         cover.append(sc)
    #
    # cycle, cc = has_cycle(adj_copy.detach().cpu().numpy())
    # cycle_orig, cc_orig = has_cycle(adj.detach().cpu().numpy())
    # print("removed graph", cycle, len(cc), cc, "\noriginal graph", cycle_orig, len(cc_orig), cc_orig)
    ajd_copy_most_connected = adj.clone()
    nodes = set(range(n))
    most_connected_nodes = []
    # while has_cycle(ajd_copy_most_connected)[0]:
    #     curr_max_cc = 0

    # node_to_cc = {}
    # for node in nodes:
    #     # if curr_max_cc > degrees[node]: continue
    #     old_arr = ajd_copy_most_connected[node].clone()
    #     ajd_copy_most_connected[node, :] = ajd_copy_most_connected[:, node] = 0
    #     node_to_cc[node] = len(has_cycle(ajd_copy_most_connected)[1])
    #     # if node_to_cc[node] > curr_max_cc: curr_max_cc = node_to_cc[node]
    #     ajd_copy_most_connected[node, :] = ajd_copy_most_connected[:, node] = old_arr
    # sorted_node_to_cc = [x[0] for x in sorted(node_to_cc.items(), key=lambda x: x[1])]
    # most_connected_node =sorted_node_to_cc[::-1]
    # most_connected_nodes.append(most_connected_node[0])
    # #    ajd_copy_most_connected[most_connected_node, :] = ajd_copy_most_connected[:, most_connected_node] = 0
    #
    # intersection = set(strong_confidence).intersection(set(most_connected_node[:len(strong_confidence) + n // 10]))
    # print("max cc", len(intersection) / len(strong_confidence), len(strong_confidence), n)
    # if min_disagree[closesed_index] != 0:
    #     df = create_dataframe_for_animation(
    #         embeddings_list,
    #         [str(0 if d in strong_confidence else (1 if prev_res[d] == assignments[closesed_index][d] else 2)) for d in
    #          range(n)],  # [str(d != d2) for d, d2 in zip(assignments[closesed_index], close_ass)],
    ...
    #     # fig.show()
    #
    # print(len(strong_confidence), strong_confidence)
    # print("size of confidence", len(cover), len(strong_confidence), len(neighbors), n, len(strong_confidence)/n, len(neighbors)/n)
    # print("degree confidnce stat", degrees[strong_confidence], degrees[strong_confidence].mean().item(), degrees.mean().item())

    distances_over_time = calc_dist_over_iteartions(adj, gcp.histories)
    plot_conf_over_time(distances_over_time, closesed_index)

    # same, diferent = last_change_compare(assignments[:closesed_index+1])
    confidance_same_pair_res = confidance_same_pair(adj, local_assignments, distances_over_time)

    last_confidance = distances_over_time[closesed_index]

    least_common_colors, conflict_sums, neighbor_dist = find_least_common_neighbor_color(adj.cpu().detach().numpy(), assignments[closesed_index], gcp.histories[closesed_index].cpu().detach().numpy(), k)

    rows = []
    columns = ["support", "confidence", "iteration"]
    adj_cpu = adj.cpu().detach().numpy()
    for i in range(closesed_index+1):
        support = find_least_common_neighbor_color(adj_cpu, assignments[i], gcp.histories[i].cpu().detach().numpy(), k)[1][0]
        for j in range(len(support)):
            rows.append([support[j], distances_over_time[i][j], i])
    df = pd.DataFrame(rows, columns=columns)

    fig = px.scatter(df, x='support', y='confidence',  animation_frame="iteration")
    # spearmanr = sci_spearmanr(conflict_sums[0], neighbor_dist)
    return

    if min_disagree[closesed_index] != 0:
        fvs = remove_cycles(adj.detach().cpu().numpy().copy())
        agreement_values_of_nodes =np.array([d==d2 for d, d2 in zip(assignments[closesed_index], close_ass)])
        agreement_values_of_hard_nodes = agreement_values_of_nodes[fvs]

        bad_hard_nodes =  np.array(fvs)[~agreement_values_of_nodes[fvs]]
        good_hard_nodes = np.array(fvs)[agreement_values_of_nodes[fvs]]
        adj_copy = adj.clone()
        adj_copy[:,good_hard_nodes] = adj_copy[good_hard_nodes,:] = 0
        second_fvs = remove_cycles(adj_copy.detach().cpu().numpy().copy())
        if len(second_fvs) != 0: print(sum(agreement_values_of_hard_nodes)/len(agreement_values_of_hard_nodes), "sad: ", len(fvs), len(bad_hard_nodes), len(second_fvs), second_fvs)
        print("Feedback Vertex Set:", agreement_values_of_hard_nodes)
        df = create_dataframe_for_animation(
            embeddings_list,
            [str(i in fvs) for i in range(n) ], closesed_index, assignments, adj, degrees, gcp)
        fig = px.scatter_3d(df, x='PCA1', y='PCA2', z="PCA3",
                            color="color",
                            animation_frame='time_step',
                            color_continuous_scale='Greys',
    ...
    # pca.fit(gcp.histories[closesed_index].clone().detach().cpu().numpy())
    # comp = pca.components_
    # mean = pca.mean_
    # if min_disagree[closesed_index] == 0:
    #     agg_comp = COMP_GOOD * COUNT_GOOD
    #     agg_mean = MEAN_GOOD * COUNT_GOOD
    #     agg_mean += mean
    #     agg_comp += comp
    #     COUNT_GOOD += 1
    #     COMP_GOOD = agg_comp/COUNT_GOOD
    #     MEAN_GOOD = agg_mean/COUNT_GOOD
    # else:
    #     agg_comp = COMP_BAD * COUNT_BAD
    #     agg_mean = MEAN_BAD * COUNT_BAD
    #     agg_comp += comp
    #     agg_mean += mean
    #     COUNT_BAD += 1
    #     COMP_BAD = agg_comp/COUNT_BAD
    #     MEAN_BAD = agg_mean/COUNT_BAD


    return

    # greedy_k, _ = greedyColoring(adj, n)
    # # sdp_k, _,_ = sdp_coloring(adj.cpu().detach().numpy())
    # sdp_k = -1
    # print("Greedy res", greedy_k, "sdp", sdp_k)
    # print("Number of connected components:", n_components)
    # print("lovasz", min_disagree[closesed_index], lovasz_theta_max_dist(adj.cpu(), features))
    # results_data["lovasz"].append(lovasz_theta_max_dist(adj.cpu(), features))
    # results_data["n_connected_components"].append(n_components)
    # results_data["greedy_k"].append(greedy_k)
    # results_data["sdp_k"].append(sdp_k)

    res = run_max_k_cut(nx.from_numpy_array(adj.cpu().detach().numpy()), k=3)
    print("max cut", res[0], "edges", adj.sum().item()/2)
    # fj_embedding, fj_ass = solve_max_3_cut_frieze_jerrum(adj.cpu().detach().numpy(), k=3)
    fj_embedding = [-1] * (max(res[2].keys()) + 1)
    for key,v in res[1].items():
        fj_embedding[key] = v.squeeze()
    fj_embedding = np.array(fj_embedding)
    fj_ass = [-1] * (max(res[2].keys()) + 1)
    for key, v in res[2].items():
        fj_ass[key] = v + 1
    fj_ass = np.array(fj_ass)
    def dist_calc(ass1, ass2):
        """
        changes ass2. does not touch ass1
        this does not check all options for some reason
        """
        ass2 = ass2.copy() if type(ass2) == type(np.array([1,2])) else np.array(ass2)
        min_fj_dist = len(ass2)
        min_k = 0
        permutations = list(itertools.permutations(list(range(k))))
        for permutation in permutations:
            per_map = {i+1:elem+1 for i, elem in enumerate(permutation)}
            ass2_changed = [per_map[i] for i in ass2]
            fj_distance = sum([int(a != c) for a, c in zip(ass1, ass2_changed)])
            if min_fj_dist > fj_distance:
                min_fj_dist = fj_distance
                min_k = per_map
        ass2[:] = np.array([per_map[i] for i in ass2])[:]
        # for k_ in range(k):
        #     fj_distance = sum([int(a != ((c + k_) % k) + 1) for a, c in zip(ass1, ass2)])
        #     if min_fj_dist > fj_distance:
        #         min_fj_dist = fj_distance
        #         min_k = k_
        # ass2[:] = (((ass2 + min_k) % k) + 1)[:]
        return min_fj_dist
    min_fj_dist = dist_calc(assignments[closesed_index], fj_ass)
    fj_is_kcol, fj_disagreement_count, fj_bad_indexes = is_k_color(adj, fj_ass.copy()-1)
    fj_closest = find_closest_kcoloring(fj_ass, adj, k)
    fj_dist_from_fjclosest = dist_calc(fj_closest, fj_ass)
    fjclosest_dist_from_model = dist_calc(assignments[closesed_index], fj_closest)
    print("fj distance", min_fj_dist)
    print("fj is colorable", fj_is_kcol, fj_disagreement_count)
    print("fj_dist_from_fjclosest", fj_dist_from_fjclosest)
    print("fjclosest_dist_from_model", fjclosest_dist_from_model)
    return

    fj_pca = PCA(3)
    fj_lower = fj_pca.fit_transform(fj_embedding)
    fg_km_assignment, _ = sklearn_k_means(fj_lower, k)
    fj_is_kcol, fj_disagreement_count, fj_bad_indexes = is_k_color(adj, fg_km_assignment.copy()-1)

    fig =  px.scatter(x=fj_lower[:,0], y=fj_lower[:,1], color=[str(a) for a in fj_ass])
    # fig.show()
    fig = px.scatter(x=fj_lower[:, 0], y=fj_lower[:, 1], color=[str(a) for a in assignments[closesed_index]])
    # fig.show()
    fig = px.scatter(x=fj_lower[:, 0], y=fj_lower[:, 1], color=[str(a) for a in fj_closest])
    # fig.show()
    # # fj_lower = pca.transform(fj_embedding)
    # # fig = px.scatter(x=fj_lower[:, 0], y=fj_lower[:, 1], color=fj_ass)
    # # fig.show()
    # px.scatter_3d(x=fj_lower[:, 0], y=fj_lower[:, 1], z=fj_lower[:, 1], color=[str(a) for a in fj_ass], title=str( round(abs(res[0] - adj.sum().item()/2), 3))+str(fj_is_kcol)).show()
    print("sdp dis and res", round(abs(res[0] - adj.sum().item()/2), 3), fj_is_kcol, fj_is_kcol)
    # px.scatter_3d(x=fj_lower[:,0], y=fj_lower[:,1], z=fj_lower[:,1], color=[str(a) for a in assignments[closesed_index]]).show()
    # px.scatter_3d(x=fj_lower[:,0], y=fj_lower[:,1], z=fj_lower[:,1], color=[str(a) for a in fj_closest]).show()
    ...
    min_distance = []
    for i in range(len(embeddings_list[closesed_index])):
        neighbors = torch.where(adj[i] > 0)[0]
        if len(neighbors) == 0:
            continue
        distances = [np.linalg.norm(embeddings_list[closesed_index][i] - embeddings_list[closesed_index][neighbor]) for neighbor in neighbors]
        min_distance.append(min(distances))
    print(f"minimum distance to a neighbor: {min(min_distance)}")
    results_data["min_dist_from_neighbors"].append(min(min_distance))

    # transformed_centroids = [(c - pca.mean_) @ pca.components_.T for c in centroids]
    transformed_centroids = centroids
    transformed_centroids = [t[:,:3] for t in transformed_centroids]
    data = []
    for time_step, cent in enumerate(transformed_centroids):
        for v_i, vector in enumerate(cent):
            data.append({
                'PC1': vector[0],
                'PC2': vector[1],
                'PC3': vector[2],
    ...
    # df = pd.DataFrame(transformed_centroids, columns=['PC1', 'PC2', 'PC3'])
    # df['c_i'] = range(len(transformed_centroids))
    fig = px.scatter(df, x='PC1', y='PC2',color="index" , animation_frame='time_step', title='Transformed Centroids Over Time')
    # fig.show()

    if min_disagree[closesed_index] == 0:
        df = df[df['time_step'] == closesed_index]
        fig = px.scatter(df, x='PC1', y='PC2', animation_frame='time_step', title='Transformed Centroids Over Time')
    #     fig.show()

    print("most comon color", most_frequent_number(assignments[closesed_index]))
    results_data["most_common_color"].append(most_frequent_number(assignments[closesed_index]))
    results_data["starting_color"] = most_frequent_number(assignments[0])
    ...
    def calculate_average_distances_per_iteration(embeddings_list, adj, ass):
        ...
        for embeddings in embeddings_list:
            ...
            for i in range(len(embeddings)):
                ...
                if len(neighbors) != 0:
                    neihbors_distances = [np.linalg.norm(embeddings[i] - embeddings[neighbor]) for neighbor in neighbors]
                avg_distances_neihobrs.append(np.mean(neihbors_distances))

                strangers = torch.where(adj[i] == 0)[0]
                strangers_distances = [0]
                if len(strangers) != 0:
                    strangers_distances = [np.linalg.norm(embeddings[i] - embeddings[stranger]) for stranger in strangers]
    ...
    avg_distances_per_iteration_neighbors, avg_distances_per_iteration_strangers, avg_distances_per_iteration_color_group = calculate_average_distances_per_iteration(embeddings_list[:closesed_index], adj, assignments[closesed_index])
    df = pd.DataFrame({
    ...
    results_data["node_distance_ordering"].append(stats.statistic)
    results_data["node_distance_ordering_p"].append(stats.pvalue)
    print(f"Regression slope: {slope}")
    results_data["regression_slope"].append(slope)


    regression_line = slope * iterations + intercept
    fig = px.line(df, x='Iteration', y=['Average Distance', 'Average Distance Strangers', 'Average Distance Color Group'],
    ...
    for i in iters:
        ...
        diff = sum(1 for a, b in zip(current_assignment, previous_assignment) if a != b)
        differences.append(diff)
    df = pd.DataFrame({'Iteration': iters, 'Difference': differences})
    fig = px.line(df, x='Iteration', y='Difference', title='Difference in Assignments Between Consecutive Iterations')
    # fig.show()
    pass
    results_data["diff_in_assignments"].append(differences)
    ...
    for i in iters:
        current_assignment = assignments[i]
        previous_assignment = assignments[i - 1]
        prev_feature = gcp.histories[i - 1].clone().detach().cpu()
        ...
        neihboring_distange_changed.append(np.mean([np.linalg.norm(prev_feature[node] - prev_feature[neighbor]) for node in changed for neighbor in range(len(adj)) if adj[node][neighbor] > 0]))
        neihboring_distange_unchanged.append(np.mean([np.linalg.norm(prev_feature[node] - prev_feature[neighbor]) for node in range(len(adj)) if node not in changed for neighbor in range(len(adj)) if adj[node][neighbor] > 0]))
        ...
        c_distanve_unchanged.append(np.mean([np.linalg.norm(clause - prev_feature[node]) for node in range(len(adj)) if node not in changed]))

        least_conflicting = [node for node in changed if current_assignment[node] in least_conflicting_color(node, previous_assignment, adj, k)]
        weighted_least_conflicting = [node for node in changed if current_assignment[node] in most_distant_color(node, previous_assignment, adj, prev_feature)]
    ...
    fig = px.line(df, x='Iteration', y=['Changed Nodes', 'Least Conflicting Changes', "Weighted Least Conflicting Changes", 'neihbors changed', 'neihbors unchanged', 'c changed', 'c unchanged'],
                  title='Nodes Changing Assignments and Least Conflicting Changes Over Iterations')
    # fig.show()
    if len(iterations_with_changes)>0:
        fig = px.line(x=iterations_with_changes, y=avg_degree_of_changed_nodes, title='Average Degree of Changed Nodes Over Iterations')
        # fig.show()
    results_data["avg_degree_of_changed_nodes"].append(avg_degree_of_changed_nodes)



    seen_in_iteration = []
    for ass in assignments[:closesed_index]:
        ass_set = set(ass)
        ass_map = {i: int(i in ass_set) for i in range(1, k + 1)}  # Use int to represent presence as 1 or 0
        seen_in_iteration.append(ass_map)
    df = pd.DataFrame(seen_in_iteration)
    df['Time'] = range(1, len(seen_in_iteration) + 1)
    df_melted = df.melt(id_vars='Time', var_name='Color', value_name='Value')
    fig = px.bar(df_melted, x='Time', y='Value', color='Color', barmode='group', title=f'Content of ass_map Over Time. {min_disagree[closesed_index]}')
    # fig.show()
    greedy_conflicts = greedy_coloring_least_conflicts(adj, k)
    is_k_col, disagree, missmatch_indexes = is_k_color(adj, np.array(greedy_conflicts))
    print("greedy conflict", disagree)
    results_data["greedy_conflicts"].append(disagree)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--check_path', type=str, default="checkpoints/simple_dist/best.pt")
    # parser.add_argument('--check_path', type=str, default="checkpoints/dist_from_one_another9/latest.pt")
    ...
    for j, b in enumerate(dl):
        ...
        closesdt_history = []
        for i in range(len(gcp.histories)):
            assignment, _ = sklearn_k_means(gcp.histories[i], k)
            is_k_col, disagree, missmatch_indexes = is_k_color(adj, assignment)
            # close_ass_iter = find_closest_kcoloring(assignment, adj, k)
            # miss_match_count = sum(x != y for x, y in zip(assignment, close_ass_iter))
            # closesdt_history.append(miss_match_count)
        ...
        closesed_index = np.argmin(min_disagree)
        # min_value = np.min(min_disagree)
        # min_indices = np.where(min_disagree == min_value)[0]
        # closesed_index = min_indices[len(min_indices) // 2]
        ...
        rand_ass = torch.randint(0, k, size=(n,)).numpy()+1
        _, rand_diss, _ = is_k_color(adj, rand_ass)
        close_rand = find_closest_kcoloring(rand_ass, adj, k)
        markers_rand = [int(a != c) for a, c in zip(rand_ass, close_rand)]
        rand_diss = sum(markers_rand)
        # print("n",n, "k", k, "diss", min_disagree[closesed_index],"|", rand_diss, "ind", closesed_index, "closest", dist,"|", rand_diss, "pred_opt", pred_int, "label", int(labels), "acc_opt", int(pred_int == labels), "orig_acc", int(old_acc), "vote", votes[closesed_index], "mean_std_count", np.mean(votes), np.std(votes), np.sum(votes))
        ...
        degrees = adj.sum(0).cpu()
        miss_match_degrees = degrees[missmatches_indexes[closesed_index]]
        # print(miss_match_degrees.numel(), miss_match_degrees.mean(), miss_match_degrees.mean(), degrees.mean(), degrees.std())
        # fig = px.scatter(x=x, y=y, title='Scatter Plot of x vs. y nodes colored by degree',
        #                  labels={'x': 'x', 'y': 'y'},
        #                  color=[f"{a}" if markers[i]==0 else "-1" for i, a in enumerate(assignments[closesed_index])])
        # fig.show()

        # positions = initialize_nodes(n)
        # positions = adjust_positions(positions, adj.clone().detach().cpu().numpy(), iterations=1000)
        # plot_positions(positions)


        degrees_np = degrees.numpy()
        degree_counts = {degree: (degrees_np == degree).sum() for degree in set(degrees_np)}
        if min_disagree[closesed_index]>=0:
            sorted_degree_counts = sorted(degree_counts.items())
            # print(sorted_degree_counts)

        groups_tensor = torch.tensor(close_ass)
        unique_groups = torch.unique(groups_tensor)
        means = []
        stds = []
        # Compute mean and std for each group
        for group in unique_groups:
            group_mask = (groups_tensor == group)
            group_numbers = degrees[group_mask]
            group_mean = torch.mean(group_numbers)
            group_std = torch.std(group_numbers)
            means.append(group_mean)
            stds.append(group_std)
        means_tensor = torch.tensor(means)
        stds_tensor = torch.tensor(stds)
        # print("\t", degrees.mean().item(), degrees.std().item(), degrees.min().item(), degrees.max().item(),
        #       "\n\t", means_tensor.tolist(),
        #       "\n\t", stds_tensor.tolist(),
        #       )

        num_avail_col = torch.from_numpy(color_support_count(adj, assignments[closesed_index]))
        groups_tensor = torch.tensor(close_ass)
        unique_groups = torch.unique(num_avail_col)
        means = []
        stds = []
        # Compute mean and std for each group
        for group in unique_groups:
            group_mask = (num_avail_col == group)
            group_numbers = degrees[group_mask]
            group_mean = torch.mean(group_numbers)
            group_std = torch.std(group_numbers)
            means.append(group_mean)
        ...
        if min_disagree[closesed_index] == 0:
            pass
            # print("\t", unique_groups.tolist(), degrees.mean().item(),
            #   "\n\t", degrees.mean().item(), degrees.std().item(), degrees.min().item(), degrees.max().item(),
            #   "\n\t", means_tensor.tolist(),
            #   "\n\t", stds_tensor.tolist(),
            #   )
        from scipy.sparse.csgraph import connected_components
        n_components, comp_labels = connected_components(csgraph=adj.cpu().detach().numpy(), directed=False, return_labels=True)
        print("Number of connected components:", n_components)
        # sdp_k, sdp_vecs = sdp_coloring(adj.cpu())
        print(min_disagree[closesed_index], lovasz_theta_max_dist(adj.cpu(), features))


        #################
        if min_disagree[closesed_index] != 0: continue
        continue
        # Perform PCA for each group
        assignment_tensor = torch.tensor(assignments[closesed_index])
        unique_groups = torch.unique(assignment_tensor)
        pca_results = []
        import pandas as pd
        for group in unique_groups:
            group_mask = (assignment_tensor == group)
            group_features = gcp.histories[closesed_index][group_mask].cpu().detach()

            pca = PCA(n_components=3)
            pca_components = pca.fit_transform(group_features.numpy())

            for i, real_i in zip(range(pca_components.shape[0]), np.arange(len(group_mask))[group_mask]):
                pca_results.append({
                    'PC1': pca_components[i, 0],
                    'PC2': pca_components[i, 1],
                    'PC3': pca_components[i, 2],
                    'Degree': degrees[group_mask][i].item(),
                    'Group': group.item(),
                    "Node": real_i,
                    "neighbors":assignments[closesed_index][adj[real_i].cpu()==1].tolist(),
                })

        # Create a DataFrame for Plotly
        df = pd.DataFrame(pca_results)
        # Plot using Plotly Express
        fig = px.scatter_3d(df, x='PC1', y='PC2', z='PC3', color='Degree', symbol='Group',
                            labels={'PC1': 'PC1', 'PC2': 'PC2',
                                    'PC3': 'PC3', "Node": "Node"},
                            title='PCA Scatter Plot by Group and Degree',hover_data={'Node': True, 'neighbors': True})
        ...
        fig.show()
        print(lovasz_theta(adj, features))

        continue
        ass_orig, cent = sklearn_k_means(features, k)
        is_k_col_orig, disagree_orig = is_k_color(adj, ass_orig)
        close_ass_orig = find_closest_kcoloring(ass_orig, adj, k)
        markers_orig = [int(a != c) for a, c in zip(ass_orig, close_ass_orig)]
        dist_orig = sum(markers_orig)
        ass = reassign_clusters_respecting_order(features, adj, k)
        is_k_col, disagree = is_k_color(adj, ass)
        close_ass = find_closest_kcoloring(ass, adj, k)
        markers = [int(a != c) for a, c in zip(ass, close_ass)]
        dist = sum(markers)

        # conflicts = 0
        # G = nx.from_numpy_array(adj.clone().detach().cpu().numpy())
        # for u, v in G.edges():
        #     if ass[u] == ass[v]:
        #         conflicts += 1

        print(n, k, acc, "\n", is_k_col, dist, disagree, "\n", is_k_col_orig, dist_orig, disagree_orig)
