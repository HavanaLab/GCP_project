...
with gpy.Env(params=options) as env:
    ...
    def find_diff_edge(Ma, CN, not_edges):
        ...
        while left <= right:
            ...
            edges1 = not_edges1[left:mid + 1]
            edges2 = not_edges2[left:mid + 1]
            Ma[edges1, edges2] = Ma[edges2, edges1] = 1

            sol = solve_csp(Ma, CN)
            if sol is None:  # diff_edge found
                # If the graph is not colorable, the diff_edge is in the left half
                right = mid - 1
                Ma[edges1, edges2] = Ma[edges2, edges1] = 0# Reset the edges
            else:
                # If the graph is colorable, the diff_edge is in the right half
                left = mid + 1

        # If we found the diff_edge, return it
        if left <= len(not_edges) - 1:
            return not_edges[left]
    ...
    def main():
        ...
        parser.add_argument('-path', default='data4', type=str, help='Save path')
