...
from model2 import GCPNet as GCPNet2
...
CHECK_POINT_PATH = f'./checkpoints/distilation'
...
def adjacency_loss(V_ret, M_vv, margin=1.0, device='cpu'):
    ...
    # Step 4: Compute loss
    loss = torch.mean(torch.max(torch.tensor(0.0, device=device), margin - distances) ** 2)
...
if __name__ == '__main__':
    ...
    gcp_old = GCPNet(embedding_size, tmax=args.tmax, device=args.device)
    gcp_old.to(args.device)
    gcp_old.eval()
    gcp_new = GCPNet2(embedding_size, tmax=args.tmax, device=args.device)
    gcp_new.to(args.device)
    if args.test:
        print("test - eval")
        gcp_new.eval()
    ...
    mse_critirion = torch.nn.MSELoss()
    ...
    if args.check_path is not None:
        ...
        gcp_old.load_all_attributes(checkpoint['model'])
    ...
    prev_train_flag = gcp_new.training
    if args.test: gcp_new.eval()
    for i in range(start_epoc, args.epochs if args.test is False else start_epoc+1):
        ...
        for j, b in enumerate(dl):
            ...
            gcp_old.forward(M_vv, M_vc, split, cn=cn)
            opt.zero_grad()
            pred, means, V_ret, C_ret = gcp_new.forward(M_vv, M_vc, split, cn=cn)
            l = critirion(means.to(DEVICE), torch.Tensor(labels).to(device=DEVICE))
            emb_loss = mse_critirion(gcp_new.history, gcp_old.history)
            l = l + emb_loss
            # dist_l = 0
            # sub_mats = decompose_to_submatrices(M_vv, split_list=split)
            # for sub_mat in sub_mats:
            #     dist_l += adjacency_loss(gcp_new.history, M_vv, margin=1.0, device=DEVICE)
            ...
            if not args.test:
                ...
                # l = l + l2norm_scaling * sum([param.norm() ** 2 for param in gcp.parameters()])
                l.backward()
                torch.nn.utils.clip_grad_norm_(gcp_new.parameters(), global_norm_gradient_clipping_ratio)
                opt.step()
        ...
        # Test
        if not args.test:
            prev_train_flag_test = gcp_new.training
            gcp_new.eval()
            loss_agg = []
            with torch.no_grad():
                test_acc = 0
                ...
                test_acc /= (j+1)
            gcp_new.train(prev_train_flag_test)
            lr_scheduler.step(np.mean(loss_agg))
        else:
            test_acc = -1
        if best < test_acc or (i % EPOC_STEP) == 0 and not args.test:
            print('Saving model')
            best = save_model(i, gcp_new, epoc_acc, epoc_loss, opt, test_acc, best)
